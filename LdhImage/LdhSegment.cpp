#include "pch.h"
#include "LdhSegment.h"
#include "LdhEnhance.h"
#include "LdhFourier.h"
#include "LdhGeometry.h"

void LdhBinarization(LdhByteImage& imgSrc, LdhByteImage& imgDst, int threshold)
{
...
int LdhBinarizationIterative(LdhByteImage& imgSrc)
{
	float hist[256] = { 0. };
	LdhHistogram(imgSrc, hist);	// 정규화된 히스토그램. hist 배열의 범위는[ 0,1].

	// 초기 임계값 셜정 - 그레이 스케일 값의 전체 평균
...
void LdhContourTracing(LdhByteImage& imgSrc, int sx, int sy, std::vector<LdhPoint>& cp)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();

	BYTE** pSrc = imgSrc.GetPixel2D();
	...
	while (1)
	{
		...
		else
		{
			...
			// 방향 정보 초기화
			cnt = 0;
			d = (d + 6) % 8; // d = d - 2와 같은 형태
		}
		// 시작 점으로 돌아왔고, 진행 방향이 초기화된 경우
		// 외곽선 추적을 끝낸다.
		if (x == sx && y == sy && d == 0)
			break;
...
void LdhMorphologyErosion(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int i, j;
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhMorphologyDilation(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int i, j; 
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhMorphologyOpening(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	LdhByteImage imgTemp;
	LdhMorphologyErosion(imgSrc, imgTemp);
	LdhMorphologyDilation(imgTemp, imgDst);
}

void LdhMorphologyClosing(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	LdhByteImage imgTemp;
	LdhMorphologyDilation(imgSrc, imgTemp);
	LdhMorphologyErosion(imgTemp, imgDst);
}

void LdhMorphologyGrayErosion(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
...
void LdhMorphologyGrayDilation(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
...
void LdhMorphologyGrayOpening(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	LdhByteImage imgTemp;
	LdhMorphologyGrayErosion(imgSrc, imgTemp);
	LdhMorphologyGrayDilation(imgTemp, imgDst);
}

void LdhMorphologyGrayClosing(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	LdhByteImage imgTemp;
	LdhMorphologyGrayDilation(imgSrc, imgTemp);
	LdhMorphologyGrayErosion(imgTemp, imgDst);
}

void LdhFourierDescriptor(LdhByteImage& img, int sx, int sy, int percent, std::vector<LdhPoint>& cp)
{
	// 외각선 좌표 구하기
	LdhContourTracing(img, sx, sy, cp);
...
void LdhInvariantMoments(LdhByteImage& img, double m[7])
{
	...
	g00 = LdhGeometricMoment(img, 0, 0);
	g10 = LdhGeometricMoment(img, 1, 0);
	g01 = LdhGeometricMoment(img, 0, 1);
	g20 = LdhGeometricMoment(img, 2, 0);
	g02 = LdhGeometricMoment(img, 0, 2);
	g11 = LdhGeometricMoment(img, 1, 1);
	g30 = LdhGeometricMoment(img, 3, 0);
	g03 = LdhGeometricMoment(img, 0, 3);
	g21 = LdhGeometricMoment(img, 2, 1);
	g12 = LdhGeometricMoment(img, 1, 2);
...
double LdhGeometricMoment(LdhByteImage& img, int p, int q)
{
	int w = img.GetWidth();
	int h = img.GetHeight();
...
bool LdhZernikeMoments(LdhByteImage& img, int n, int m, double& zr, double& zi)
{
	...
	LdhByteImage imgDst;
	LdhResizeBilinear(img, imgDst, ZM_SIZE, ZM_SIZE);
	BYTE** pDst = imgDst.GetPixel2D();
	...
	for( y = 0; y < ZM_SIZE; y++)
		for (x = 0; x < ZM_SIZE; x++)
		{
			...
			if (dist <= ZM_RADIUS)
			{
				...
				zi -= ((pDst[y][x] / 255.) * zm_basis_imag[y][x]);	// Complex conjgate
			}
		}
...
LdhPoint LdhTemplateMatching(LdhByteImage& imgSrc, LdhByteImage& imgTmpl, LdhIntImage& imgMap)
{
	...
	BYTE** pSrc = imgSrc.GetPixel2D();
	BYTE** pTmpl = imgTmpl.GetPixel2D();
	int** pMap = imgMap.GetPixel2D();
...
int LdhLabeling(LdhByteImage& imgSrc, LdhIntImage& imgDst, std::vector<LdhLabelInfo>& labels)
{
	...
	// ---------------------------------------------------
	// 첫 번쨰 스캔 - 초기 레이블 지정 및 등가 테이블 생성
	// ---------------------------------------------------

	register int i, j;
	int label = 0, maxl, minl, max_eq, min_eq;
	...
	for(j = 1; j < h; j++)
		for (i = 1; i < w; i++)
		{
			if (pSrc[j][i] != 0)
			{
				...
				{
					if (pMap[j - 1][i] == pMap[j][i - 1]) // 두 레이블이 서로 같은 경우
						pMap[j][i] = pMap[j - 1][i];
					else // 두레이블이 서로 다른 경우
					{
				...
				else if (pMap[j - 1][i] != 0) // 바로 위 픽셀만 레이블이 존재할 경우
				{
					pMap[j][i] = pMap[j - 1][i];
				}
				else if (pMap[j][i - 1] != 0) // 바로 왼쪽 픽셀만 레이블이 존재할 경우
				{
					pMap[j][i] = pMap[j][i - 1];
				}
				else // 이웃에 레이블이 존재하지 않으면 새로운 레이블을 부여
				{
					label++;
					pMap[j][i] = label;
	...
	// -------------------------------------------------------------------
	// 두번째 스킨 - 등가 테이블을 이용하여 모든 픽셀에 고유의 레이블 부여
	// -------------------------------------------------------------------

	imgDst.CreateImage(w, h);
	int** pDst = imgDst.GetPixel2D();
	...
	for(j = 1; j < h; j++)
		for (i = 1; i < w; i++)
		{
			if (pDst[j][i] != 0)
			{
				...
				pLabel->pixels.push_back(LdhPoint(i, j));
				pLabel->cx += i;
				pLabel->cy += j;
