...
void LdhTranslate(LdhByteImage& imgSrc, LdhByteImage& imgDst, int sx, int sy)
{
...
void LdhResizeNearest(LdhByteImage& imgSrc, LdhByteImage& imgDst, int nw, int nh)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhResizeBilinear(LdhByteImage& imgSrc, LdhByteImage& imgDst, int nw, int nh)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhResizeCubic(LdhByteImage& imgSrc, LdhByteImage& imgDst, int nw, int nh)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	...
	for(j = 0; j < nh; j++)
		for (i = 0; i < nw; i++)
		{
			...
			pDst[j][i] = static_cast<int>(limit(v + 0.5));

		}

}

void LdhRotate(LdhByteImage& imgSrc, LdhByteImage& imgDst, double angle)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	...
	// 회전후 생성되는 영상의 크기를 계산
	// 4개의 코너 포인트의 이동 좌표를 계싼하여 최대, 최소 점의 차이를 구한다.
...
void LdhRotate90(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhRotate180(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	imgDst.CreateImage(w,h);
...
void LdhRotate270(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	imgDst.CreateImage(h, w);
...
void LdhMirror(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhFlip(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
