...
#define CHECK_WEAK_EDGE(x,y)\
	if(pEdge[y][x] == WEAK_EDGE) {\
		...
		strong_edges.push_back(LdhPoint(x,y));\
	}

void LdhEdgeRoberts(LdhByteImage& img, LdhByteImage& imgEdge)
{
	int w = img.GetWidth();
	int h = img.GetHeight();
	...
	BYTE** p1 = img.GetPixel2D();
	BYTE** p2 = imgEdge.GetPixel2D();
...
void LdhEdgePrewitt(LdhByteImage& img, LdhByteImage& imgEdge)
{
	int w = img.GetWidth();
	int h = img.GetHeight();
	...
	for(j = 1; j< h-1;j++)
		for (i = 1; i < w - 1; i++)
		{
			...
			h2 = - p1[j - 1][i - 1] + p1[j - 1][i + 1]
				 - p1[j	   ][i - 1] + p1[j    ][i + 1]
				 - p1[j + 1][i - 1] + p1[j + 1][i + 1];
...
void LdhEdgeSobel(LdhByteImage& img, LdhByteImage& imgEdge)
{
...
void LdhEdgeCanny(LdhByteImage& imgSrc, LdhByteImage& imgEdge, float sigma, float th_low, float th_high)
{
	register int i, j;
	...
	LdhFloatImage imgGaussian(w, h);
	LdhFilterGaussian(imgSrc, imgGaussian, sigma);

	// 2. 그래디언트 구하기 (크기 & 방향)

	LdhFloatImage imgGx(w, h); // gradient of x
	LdhFloatImage imgGy(w, h); // gradient of y
	LdhFloatImage imgMag(w, h); // magnitude of gradient

	float** pGauss = imgGaussian.GetPixel2D();
	float** pGx = imgGx.GetPixel2D();
	float** pGy = imgGy.GetPixel2D();
	float** pMag = imgMag.GetPixel2D();
	...
	std::vector<LdhPoint> strong_edges;
			...
			if (pMag[j][i] > th_low)
			{
				...
				if (local_max)
				{
					if (pMag[j][i] > th_high)
					{
						...
						strong_edges.push_back(LdhPoint(i, j));
					}
					else
						pEdge[j][i] = WEAK_EDGE;
				}
...
void LdhHoughLine(LdhByteImage& img, std::vector<LdhLineParam>& lines, int threshold)
{
	register int i, j;
	...
	LdhIntImage imgAcc(num_ang, num_rho);
	int** pAcc = imgAcc.GetPixel2D();
		...
		for (n = 0; n < num_ang; n++)
		{
			...
			if (value > 1)
			{
				if (value >= pAcc[m - 1][n] && value >= pAcc[m - 1][n + 1] &&
					...
					value >= pAcc[m][n - 1] && value >= pAcc[m - 1][n - 1])
				{
					lines.push_back(LdhLineParam(m - (num_rho / 2), n * PI / num_ang, pAcc[m][n]));
				}
			}
		}
...
void LdhDrawLine(LdhByteImage& img, LdhLineParam line, BYTE c)
{
	int w = img.GetWidth();
	int h = img.GetHeight();
...
void LdhHarrisCorner(LdhByteImage& img, std::vector<LdhPoint>& corners, double th)
{
	...
	LdhFloatImage imgDx2(w, h);
	LdhFloatImage imgDy2(w, h);
	LdhFloatImage imgDxy(w, h);

	float** dx2 = imgDx2.GetPixel2D();
	float** dy2 = imgDy2.GetPixel2D();
	float** dxy = imgDxy.GetPixel2D();
	...
	LdhFloatImage imgGdx2(w, h);
	LdhFloatImage imgGdy2(w, h);
	LdhFloatImage imgGdxy(w, h);

	float** gdx2 = imgGdx2.GetPixel2D();
	float** gdy2 = imgGdy2.GetPixel2D();
	float** gdxy = imgGdxy.GetPixel2D();
	...
	LdhFloatImage imgCrf(w, h);
	float** crf = imgCrf.GetPixel2D();
		...
		for (i = 2; i < w - 2; i++)
		{
			...
			if(cvf_value > th)
				if (cvf_value > crf[j - 1][i] && cvf_value > crf[j - 1][i + 1] &&
					...
					corners.push_back(LdhPoint(i, j));
				}
		}
