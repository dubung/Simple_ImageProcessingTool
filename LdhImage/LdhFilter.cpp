...
void LdhFilterMean(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
...
void LdhFilterWeightedMean(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhFilterGaussian(LdhByteImage& imgSrc, LdhFloatImage& imgDst, float sigma)
{
	...
	LdhFloatImage imgMask(dim, 1); // dim,1인 float 형 이미지 생성
	...
	LdhFloatImage imgBuf(w, h);
	float** pBuf = imgBuf.GetPixel2D();
...
void LdhFilterLaplacian(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	...
	for(j = 1; j < h - 1; j++)
		for (i = 1; i < w - 1; i++)
		{
			sum = pSrc[j - 1][i] + pSrc[j + 1][i] + pSrc[j][i - 1] + pSrc[j][i + 1] - 4 * pSrc[j][i];

			pDst[j][i] = static_cast<BYTE>(limit(sum + 128));
		}
}

void LdhFilterUnsharpMask(LdhByteImage& imgSrc, LdhByteImage& imgDst) {
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhFilterHighboost(LdhByteImage& imgSrc, LdhByteImage& imgDst, float alpha)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void LdhNoiseGaussian(LdhByteImage& imgSrc, LdhByteImage& imgDst, int amount)
{
...
void LdhNoiseSaltNPepper(LdhByteImage& imgSrc, LdhByteImage& imgDst, int amount)
{
	int size = imgSrc.GetSize();
	imgDst = imgSrc;
	BYTE* pDst = imgDst.GetPixels();
...
void LdhFilterMedian(LdhByteImage& imgSrc, LdhByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	...
	for(j = 1; j < h-1; j++)
		for (i = 1; i < w - 1; i++)
		{
			...
			pDst[j][i] = m[4]; // 중간 값을 넣어줌
		}

}

void LdhFIlterDiffusion(LdhByteImage& imgSrc, LdhFloatImage& imgDst, float lambda, float k, int iter)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();

	LdhFloatImage imgCpy;
	imgCpy.Convert(imgSrc);

	imgDst = imgCpy;

	float** pCpy = imgCpy.GetPixel2D();
	float** pDst = imgDst.GetPixel2D();
