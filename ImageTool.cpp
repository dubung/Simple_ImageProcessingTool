...
CImageToolApp::CImageToolApp() noexcept :m_pNewDib(NULL), m_pImageDocTemplate(NULL)
{
...
BOOL CImageToolApp::InitInstance()
{
	...
	/*pDocTemplate*/ m_pImageDocTemplate = new CMultiDocTemplate(IDR_ImageToolTYPE,
...
void CImageToolApp::OnUpdateEditPaste(CCmdUI* pCmdUI)
{
	...
	// CF_DIB 를 넌겨주면 DIB 형식인 경우 TRUE 아니면 FALSE를 반환

}


// 풀력창에 영상 처리 정보 문자열 출력
void AfxPrintInfo(CString message)
{
	CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd(); // Main Frame 의 주소를 받아온다
...
void CImageToolApp::OnWindowCloseall()
{
	...
	// void ClosAllDocuments(BOOL bEndSession); 
	// bEndSession 에 TRUE 와 FALSE 둘다 동작에는 차이가 없다
}
